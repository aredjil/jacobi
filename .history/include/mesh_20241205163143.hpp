...
public:
     ...
     */
    template <typename F>
    Mesh(const int n, F &boundary_conditions, const MPI_Comm &comm);

    /* Print to a file in parallel*/
    void print_to_file(const int count, const std::string &filename = "output");
    /* Print local data in a matrix format*/
    std::ostream &print(std::ostream &os, const std::vector<T> &matrix);
};
/**
...
template <typename T>
template <typename F>
Mesh<T>::Mesh(const int n, F &boundary_conditions, const MPI_Comm &comm) : size(n), comm(comm)
{
    /**
...
template <typename T>
std::ostream &Mesh<T>::print(std::ostream &os, const std::vector<T> &matrix)
{
...
template <typename T>
void Mesh<T>::print_to_file(const int count, const std::string &filename) 
{
