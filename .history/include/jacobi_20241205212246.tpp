...
#pragma omp parallel for
            for (int j = 1; j < m.n_loc + 1; ++j)
            {
                ...
                vec_nfield_ptr[(m.n_loc) * (n + 2) + j] = (1.0 / 4) * (vec_cfield_ptr[(m.n_loc - 1) * (n + 2) + j] + vec_cfield_ptr[(m.n_loc + 1) * (n + 2) + j] + m.current_field[(m.n_loc) * (n + 2) + j - 1] + m.current_field[(m.n_loc) * (n + 2) + j + 1]);
            }
        }
        // Swap the current field values with the new field values before the next iteration
